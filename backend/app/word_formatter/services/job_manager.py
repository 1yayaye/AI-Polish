...
class JobManager:
    ...
    def __init__(
        ...
        self.job_retention_hours = max(1, int(job_retention_hours))
        self._semaphore = asyncio.Semaphore(self.max_concurrent_jobs)
        self._job_retention = timedelta(hours=self.job_retention_hours)
    ...
    async def cleanup_old_jobs(self) -> int:
        ...
        to_remove = [
            ...
            if job.updated_at < cutoff and job.status not in {JobStatus.PENDING, JobStatus.RUNNING}
        ]
...
def get_job_manager() -> JobManager:
    ...
    if _job_manager is None:
        _job_manager = JobManager(
            max_concurrent_jobs=settings.WORD_FORMATTER_MAX_CONCURRENT_JOBS,
            job_retention_hours=settings.WORD_FORMATTER_JOB_RETENTION_HOURS,
        )
    return _job_manager
