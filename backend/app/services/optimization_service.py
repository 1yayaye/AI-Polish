...
from app.services.billing_service import mark_workspace_charge_succeeded, refund_workspace_charge
...
class OptimizationService:
    ...
    async def start_optimization(self):
        ...
        try:
            ...
            self.session_obj.failed_segment_index = None
            mark_workspace_charge_succeeded(self.db, session=self.session_obj)
            self.db.commit()
            ...
            refund_workspace_charge(self.db, session=self.session_obj, reason=error_msg)
            self.db.commit()
            raise
        finally:
            # 释放并发权限
