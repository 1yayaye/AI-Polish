...
const createResourceAccessToken = async (resourceType, resourceId, action) => {
...
export const optimizationAPI = {
  getBilling: () => api.get('/optimization/billing', {
    timeout: 10000,
  }),
  ...
  getStreamUrl: async (sessionId) => {
    const accessToken = await createResourceAccessToken('optimization_session', sessionId, 'stream');
    const baseUrl = api.defaults.baseURL || '/api';
    return `${baseUrl}/optimization/sessions/${sessionId}/stream?access_token=${encodeURIComponent(accessToken)}`;
  },
};
...
// Model Profiles API (public, for user selection)
export const modelProfilesAPI = {
  list: () => api.get('/optimization/model-profiles'),
};

// Admin Model Profiles API
const getAdminAuthConfig = (adminToken) => ({
  headers: { Authorization: `Bearer ${adminToken}` },
});

export const adminModelProfilesAPI = {
  list: (adminToken) => api.get('/admin/model-profiles', getAdminAuthConfig(adminToken)),
  create: (data, adminToken) => api.post('/admin/model-profiles', data, getAdminAuthConfig(adminToken)),
  update: (id, data, adminToken) => api.put(`/admin/model-profiles/${id}`, data, getAdminAuthConfig(adminToken)),
  delete: (id, adminToken) => api.delete(`/admin/model-profiles/${id}`, getAdminAuthConfig(adminToken)),
  setDefault: (id, adminToken) => api.post(`/admin/model-profiles/${id}/set-default`, null, getAdminAuthConfig(adminToken)),
};
...
export const wordFormatterAPI = {
  ...
  getDownloadUrl: async (jobId) => {
    const accessToken = await createResourceAccessToken('word_job', jobId, 'download');
    const baseUrl = api.defaults.baseURL || '/api';
    return `${baseUrl}/word-formatter/jobs/${jobId}/download?access_token=${encodeURIComponent(accessToken)}`;
  },

  // SSE stream URL
  getStreamUrl: async (jobId) => {
    const accessToken = await createResourceAccessToken('word_job', jobId, 'stream');
    const baseUrl = api.defaults.baseURL || '/api';
    return `${baseUrl}/word-formatter/jobs/${jobId}/stream?access_token=${encodeURIComponent(accessToken)}`;
  },
  ...
  getPreprocessStreamUrl: async (jobId) => {
    const accessToken = await createResourceAccessToken('word_job', jobId, 'stream');
    const baseUrl = api.defaults.baseURL || '/api';
    return `${baseUrl}/word-formatter/preprocess/${jobId}/stream?access_token=${encodeURIComponent(accessToken)}`;
  },
