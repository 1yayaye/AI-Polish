...
import { optimizationAPI, modelProfilesAPI } from '../api';
import { formatChinaDate } from '../utils/timezone';
...
const WorkspacePage = () => {
  ...
  // 加载可用模型列表
  useEffect(() => {
    modelProfilesAPI.list().then(res => {
      setModelProfiles(res.data);
      const defaultProfile = res.data.find(p => p.is_default);
      if (defaultProfile) {
  ...
  const loadBilling = useCallback(async () => {
    try {
      ...
      const response = await optimizationAPI.getBilling();
  ...
  useEffect(() => {
    ...
    loadQueueStatus();
    loadBilling();
  }, [loadSessions, loadQueueStatus, loadBilling]);
  ...
  const pricePer10kCents = Number(billing?.workspace_price_per_10k_cents || 0);
  const workspaceBalanceCents = Number(billing?.workspace_balance_cents || 0);
  const priceConfigured = Boolean(billing?.price_configured) && pricePer10kCents > 0;
  const estimatedBillingCents = useMemo(() => {
    if (!text.trim() || !priceConfigured) {
      return 0;
    }
    return Math.max(1, Math.ceil(text.length * pricePer10kCents / 10000));
  }, [text, pricePer10kCents, priceConfigured]);
  const hasEnoughBalance = estimatedBillingCents === 0 || workspaceBalanceCents >= estimatedBillingCents;
  const startDisabledReason = (() => {
    if (!text.trim()) return '';
    if (!priceConfigured) return 'Workspace 计费价格未配置，请联系管理员配置后再开始。';
    if (!hasEnoughBalance) return 'Workspace 余额不足，请联系管理员充值后再开始。';
    return '';
  })();
  const isStartDisabled = !text.trim() || Boolean(activeSession) || isSubmitting || !priceConfigured || !hasEnoughBalance;
  ...
  const handleStartOptimization = useCallback(async () => {
    ...
    if (!priceConfigured) {
      toast.error('Workspace 计费价格未配置，请联系管理员配置后再开始');
      return;
    }

    if (!hasEnoughBalance) {
      toast.error('Workspace 余额不足，请联系管理员充值后再开始');
      return;
    }
  ...
  }, [text, processingMode, selectedProfileId, isSubmitting, loadSessions, loadBilling, priceConfigured, hasEnoughBalance]);
            ...
            <div className="bg-white rounded-2xl shadow-ios p-5">
                    ...
                    {modelProfiles.map((profile) => (
                      ...
                      >
                        <input
                          ...
                          className="mr-3 w-4 h-4 text-ios-blue focus:ring-ios-blue border-gray-300"
                        />
                        <div className="min-w-0">
                          <div className={`font-medium text-[14px] truncate ${selectedProfileId === profile.id ? 'text-ios-blue' : 'text-black'}`}>
                            {profile.name}
                          </div>
                          <div className="text-[12px] text-ios-gray truncate">
                            {profile.model}
                          </div>
                        </div>
                      </label>
              ...
              <div className="mt-4 rounded-xl border border-gray-100 bg-gray-50 p-4">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                  <div>
                    <div className="text-[12px] text-ios-gray mb-1">Workspace 余额</div>
                    <div className="font-semibold text-black">
                      {isLoadingBilling ? '加载中...' : formatCents(workspaceBalanceCents)}
                    </div>
                  </div>
                  <div>
                    <div className="text-[12px] text-ios-gray mb-1">每万字单价</div>
                    <div className="font-semibold text-black">
                      {priceConfigured ? formatCents(pricePer10kCents) : '未配置'}
                    </div>
                  </div>
                  <div>
                    <div className="text-[12px] text-ios-gray mb-1">当前字数</div>
                    <div className="font-semibold text-black">{text.length} 字</div>
                  </div>
                  <div>
                    <div className="text-[12px] text-ios-gray mb-1">预估费用</div>
                    <div className={`font-semibold ${!hasEnoughBalance && text.trim() ? 'text-ios-red' : 'text-black'}`}>
                      {text.trim() && priceConfigured ? formatCents(estimatedBillingCents) : '--'}
                    </div>
                  </div>
                </div>
                {startDisabledReason && (
                  <div className="mt-3 flex items-start gap-2 rounded-lg bg-red-50 px-3 py-2 text-[13px] text-ios-red">
