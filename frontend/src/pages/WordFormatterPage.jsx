...
const WordFormatterPage = () => {
  ...
  const startSSE = async (jobId) => {
    ...
    let url;
    try {
      url = await wordFormatterAPI.getStreamUrl(jobId);
    } catch (error) {
      toast.error(error.response?.data?.detail || 'Failed to open progress stream');
      loadJobs();
      return;
    }
  ...
  const handleDownload = async (job) => {
    ...
    try {
      const url = await wordFormatterAPI.getDownloadUrl(job.job_id);
      window.open(url, '_blank');
    } catch (error) {
      toast.error(error.response?.data?.detail || 'Failed to create download link');
    }
  };
